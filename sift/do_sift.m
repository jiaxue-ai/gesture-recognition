...
%   开始工作
%
fprintf('start to get sift feature') ; tic ; 

fprintf('SIFT:use DoG ...\n') ; tic ; 

scalespace = do_gaussian(I,sigmaN,O,S,omin,-1,S+1,sigma0) ;

fprintf('                time for gaussin: (%.3f s)\n',toc) ; tic ; 

difofg = do_diffofg(scalespace) ;

fprintf('               subtract: (%.3f s)\n',toc) ;

for o=1:scalespace.O
    
    
	fprintf('CS5240 -- SIFT: compute pairs  %d\n', o-1+omin) ;
                tic ;
