...
for i=1:con_step:X
    for j=1:con_step:Y
        if contour_img(i,j)~=0
            if sqrt((cols-i)^2+(rows-j)^2)>1.9*r&&j>cols&&sqrt((cols-i)^2+(rows-j)^2)<3*r;
                finger_img(i,j)=1;
            end
        end
    end
...
if(finger_num==0)
    ...
    if(finger_num==4)
        ...
        if(node==0)
            disp('four'),title('four')
        end
    elseif(finger_num==5)
        disp('five'),title('five')
    end
    if(finger_num==2)
        for i=1:angle_num
    if (angle(i)~=0)
    ...
    if(finger_num==3)
        ...
        if(largeangle==2)
            disp('rock'),title('rock')
        elseif(largeangle==1)
            disp('three'),title('three')
        else
            disp('w'),title('w')
        end
    end
...
% cen_img=zeros(X,Y);
% con_step=2;
% for i=1:con_step:X-1
%     for j=1:con_step:Y-1
%         if (fin_segimg(i,j)~=0)
%             min_dist=X*Y;
%             for x=1:con_step:X-1
%                 for y=1:con_step:Y-1
%                     if(contour_img(x,y)~=0)
% %                         p1=[i j];
% %                         p2=[x y];
% %                         dist=norm(p1-p2);
...
%     for j=1:con_step:Y-1
%         if contour_img(i,j)~=0
%             if sqrt((rows-i)^2+(cols-j)^2)>2*r&&j>cols;
%                 finger_img(i,j)=1;
%             end
%         end
%     end
